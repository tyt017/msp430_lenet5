    ...
    if (*array == 0){  // 1st element of the array tells the model type
        ...
        while (array < MODEL_ARRAY_END){
            ...
            if (*array == DENSE_LAYER){
                ...
                else if (activation == TANH_ACTIVATION){
                    dense(output, input, &kernel, &bias, &fp_tanh, FIXED_POINT_PRECISION);
                }
                else{
            ...
            /* layer class 2 - Conv2D */
                ...
                if (activation == RELU_ACTIVATION){
                    ...
                    conv2d(output, input, &filters, numFilters, numChannels, bias_array, &fp_tanh, FIXED_POINT_PRECISION, stride_numRows, stride_numCols, padding);
                }
                else{
            ...
            /* layer class 6 - AveragePooling2D */
            else if (*array == AVGPOOLING2D_LAYER){
                uint16_t pool_numRows = *(array + 1);
                uint16_t pool_numCols = *(array + 2);
                stride_numRows = *(array + 3); //default stride=2
                stride_numCols = *(array + 4);//default stride=2
                padding = *(array + 5);
                array += 6;
                ...
                avgpooling_filters(output, input, numFilters, pool_numRows, pool_numCols);
            }
