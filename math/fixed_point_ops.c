...
int16_t fp_sqrt(int16_t x, uint16_t precision) {
    if (x < 0) {
        return 0;
    }

    int16_t hundred = int_to_fp32(100, precision);
//    int16_t ten = int_to_fp(10, precision);
    volatile int16_t a = x;
    volatile int16_t n = 1;

    while (a > hundred) {
//        a = fp_div(a, hundred, precision);
//        n = fp_mul(n, ten, precision);
        a /= 100;
        n *= 10;
    }

    const int16_t one256 = 1 << (precision - 8);
    const int16_t one32 = 1 << (precision - 5);
    const int16_t oneEight = 1 << (precision - 3);
    const int16_t oneHalf = 1 << (precision - 1);
    const int16_t two = 1 << (precision + 1);
    const int16_t eight = 1 << (precision + 3);
    const int16_t thirtyTwo = ((int16_t) 1) << (precision + 5);

    volatile int16_t sqrtPart = 0;

    if (a <= one256) {
        sqrtPart = fp_add(a << 4, 1 << (precision - 6));  // 16x + 1/64
    } else if (a <= one32) {
        sqrtPart = fp_add(a << 2, 1 << (precision - 5));  // 4x + 1/16
    } else if (a <= oneEight) {
        sqrtPart = fp_add(a << 1, 1 << (precision - 3));  // 2x + 1/8
    } else if (a <= oneHalf) {
        sqrtPart = fp_add(a, 1 << (precision - 2));  // x + 1/4
    } else if (a <= two) {
        sqrtPart = fp_add(a >> 1, (int32_t) oneHalf);  // x/2 + 1/2
    } else if (a <= eight) {
        sqrtPart = fp_add(a >> 2, 1 << precision);  // x/4 + 1
    } else if (a <= thirtyTwo) {
        sqrtPart = fp_add(a >> 3, 1 << (precision + 1));  // x/8  + 2
    } else {
        sqrtPart = fp_add(a >> 4, 1 << (precision + 2));  // x/16 + 4
    }

    return sqrtPart * n;
    // return fp_mul(sqrtPart, n, precision);
}
